...
case class Config(input: File = new File("src"),
                  ...
                  sassOutputSubDirectory: String = "css")
...
object ConfigParser extends scopt.OptionParser[Config]("extemplify") {
  ...
  opt[String]("sassOutputSubDirectory").action((x, c) => c.copy(sassOutputSubDirectory = x)).text("configures the sub-directory used for generated CSS files (defaults to 'css')")
